...
package org.baicaix.single {
	 ...
	 * @author dengyang
	 */
	public class RangeImpl extends EventDispatcher implements Range {
		//------------------------------------
		...
		public function RangeImpl(map: Map, range : Rectangle = null) {
			this._map = map;
			this._range = range;
			if(range != null) {
		 ...
		 * 清除邊框
		 */
		private function clearCellRim(tile : Tile, index : int, array : Array) : void {
			tile.clearRim();
		}
		 ...
		 * 粘贴区域
		 */
		public function pasteRange(baseRange : Range, sourceCellRange : Range) : void {
			//跟基准粘贴区域的偏移
			...
			//粘貼
			for each (var sourcetile : Tile in sourceCellRange.cells) {
				...
				var targettile : Tile = this._map.layers[0].getTile(targetX, targetY);
				//超出边界的处理
		...
		public function cancelRange() : void {
			for each (var tile : Tile in _tiles) {
				tile.rim = Tile.NONE_RIM;
			}
			refreshRim();
		}
		
		private function refreshRim() : void {
			dispatchEvent(new CellEvent(CellEvent.DRAW_RIM, {range : _range}));
		}
		
		private function refreshCell() : void {
			dispatchEvent(new CellEvent(CellEvent.RELOAD_CELL, {range : _range}));
		}
...
//			findRightCells().forEach(clearCellRim);
			dispatchEvent(new CellEvent(CellEvent.CLEAR_RIM, {range : _range}));
		}
