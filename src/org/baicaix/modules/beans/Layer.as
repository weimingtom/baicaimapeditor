...
package org.baicaix.modules.beans {
	...
	public class Layer {
		...
		public static const DEFAULT_TILE : Tile = new Tile();
		public var tiles : Object = {};
		
		public function Layer(parent : Map=null, z : int = -2000, name : String = "default name") {
			this.z = z;
			this.name = name;
			this._parent = parent;
		}
		...
		public function getTile(x : int, y : int) : Tile {
			var tile : Tile =  this.tiles[createKey(x, y)];
			return tile == null ? DEFAULT_TILE : tile;
		}
		
		public function getOrCreateTile(x : int, y : int) : Tile {
			var tile : Tile = this.tiles[createKey(x, y)];
			return tile != null ? tile : createTile(x, y);
		}
		
		private function createTile(x : int, y : int) : Tile {
			var key : String = createKey(x, y);
			var tile : Tile = new Tile(_parent);
			tile.x = x;
			tile.y = y;
			tiles[key] = tile;
			return tile;
